	...
	float *rect_B_x, float *rect_B_y, float *rect_B_w, float *rect_B_h, float *rect_B_speed_x, float *rect_B_speed_y, float *b_inv_mass)
{
	...
	relative_speed_x -= *rect_A_speed_x; // relative_vel X
	relative_speed_y -= *rect_A_speed_y; // relative_vel Y
...
void old_collision(float *box_x, float *box_y, float *box_speed_x, float *box_speed_y, float *box_w, float *box_h)
{
...
int main(int argc, char**argv)
{
	...
	for (;;)
	{
		...
		// TO CALL OLD COLLISION
		/*
		for (int i = 0; i < n_current_boxes; i++)
		{
			//old_collision(&box[i].x, &box[i].y, &box[i].speed_x, &box[i].speed_y, &box[i].w, &box[i].h);
			//	printf("\nEnviar %d %d %d %d", box[i].x, box[i].y, box[i].speed_x, box[i].speed_y);
		}
		*/
		...
		for (int i = 0; i < n_current_boxes; i++)
		{
			if (box[i].count_hits > n_max_hits)
			{
				...
				//new_current_boxes = n_current_boxes - 1;
			}
				
		}


		//n_current_boxes = new_current_boxes;
